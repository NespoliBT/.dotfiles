...
const changeTheme = (t) => {
    Utils.exec(`cp ${t.scss} ${App.configDir}/scss/variables.scss`)
    Utils.exec(`swww img ${t.background} --transition-type none`)
    Utils.execAsync(`cp ${t.hyprColors} ${hyprPath}/hypr-colors.conf`).catch(err => print(err));
}

const setTheme = (t) => {
    Utils.execAsync(`cp ${t.background} ${hyprPath}/background.jpg`).catch(err => print(err));
    Utils.execAsync(`cp ${t.image} ${hyprPath}/image.jpg`).catch(err => print(err));
    Utils.execAsync("ags --toggle-window 'themeChanger'").catch(err => print(err));
